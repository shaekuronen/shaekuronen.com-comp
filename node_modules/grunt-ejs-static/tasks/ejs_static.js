...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    // set default value of false for parent_dir option
    options.parent_dir = options.parent_dir || false;

    // set default value of true for underscores_to_dashes option
    if (options.underscores_to_dashes === undefined) {
      options.underscores_to_dashes = true;
    } else {
      options.underscores_to_dashes = options.underscores_to_dashes;
    }

    // set default value of .html for file_extension option
    options.file_extension = options.file_extension || '.html' 
    ...
    function create_html_files(data_object) {
      ...
      for (var key in data_object) {
        ...
        // if the page is a category page, build the category dir structure
          ...
          // if the parent_dir option is true
          if (options.parent_dir) {

            // create the file as an index.html file inside a parent directory
            var path_to_file = options.dest + '/' + data_object[key].category + '/' + key + '/index' + options.file_extension;
            underscores_to_dashes(path_to_file);
            grunt.log.debug('path to html file is ' + path_to_file);           
    ...
    function underscores_to_dashes(item) {
      ...
      if (options.underscores_to_dashes) {
        ...
        grunt.log.debug('underscores_to_dashes was set to false and processed ' + item);
        return item;
