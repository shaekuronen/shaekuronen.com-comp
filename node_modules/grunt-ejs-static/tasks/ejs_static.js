...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    function create_html_files(data_object) {
      ...
      for (var key in data_object) {
        ...
        // the page is root level
          ...
          // render the template
          render_template(path_to_file, the_data);
    ...
    // RENDER THE TEMPLATE
    function render_template(path_to_template, this_data) {
      ...
      this_data.filename = path_to_template;
      grunt.log.debug('THIS DATA FILENAME IS ' + this_data.filename);

      // get the template
      var the_template = grunt.file.read(path_to_template);
      grunt.log.debug('Read template from ' + path_to_template);

      // render the template as html
      var the_rendered_template = ejs.render(the_template, this_data);

      // write the compiled template to the destination directory
      grunt.file.write(path_to_template, the_rendered_template);
      grunt.log.debug('Rendered HTML to ' + path_to_template);
