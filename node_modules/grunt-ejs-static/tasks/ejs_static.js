  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    if (options.path_to_data) {
      ...
      if (options.global_data) {
        var global_data = grunt.file.readJSON(options.path_to_data);
    ...
    // this combines global data, page data from pages.json, and any other data from a different json file
    function prepare_data(key, data_object, global_data, path_to_layout) {
      ...
      if (global_data) {
        grunt.log_debug('there is global_data');
        var the_global_data = grunt.file.readJSON(global_data);
        var the_stringified_global_data = JSON.stringify(the_global_data, null, 2);
        grunt.log.debug('stringified global data is ' + the_stringified_global_data);
        // grunt.log.debug(global_data);
        the_data.global = the_global_data;
        // var stringified_the_data = JSON.stringify(the_data, null, 2);
        // grunt.log.debug('stringified global data is ' + stringified_the_data);
        // grunt.log.debug('Added global data to data object');
      }  

      // add any other data specified in pages.json
      if (the_data.path_to_data) {
        var other_data = grunt.file.readJSON(the_data.path_to_data);
        var stringified_other_data = JSON.stringify(other_data, null, 2);
        grunt.log.debug('the key for other data is ' + key);
        grunt.log.debug('stringified data is ' + stringified_other_data);
        the_data[key] = other_data;
      }
