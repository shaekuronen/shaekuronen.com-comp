...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    function prepare_data(key, data_object, global_data, path_to_layout) {
      ...
      if (global_data) {
        this_page_data.global = global_data;
      }  

      // add any additional data specified in pages declaration json file
      if (this_page_data.path_to_data) {

        // var additional_page_data = grunt.file.readJSON(this_page_data.path_to_data);
    ...
    function data_type_router(key, data_object, data_item) {
      ...
      // http://stackoverflow.com/questions/8834126/how-to-efficiently-check-if-variable-is-array-or-object-in-nodejs-v8
        ...
        // grunt.fail.warn('path_to_data must be a string or an array');

        grunt.log.debug('data_type_router(): data item is an object instead of required string or array');

        // grunt.log.debug('stringified object is ' + JSON.stringify(data_item, null, 2));

        // grunt.log.debug('the data item is an object');

        // Object.keys(data_item).forEach(function(the_key) {

        //   var this_data = grunt.file.readJSON(the_key);

        //   grunt.log.debug('the VVVVVVVVVVVVvalue for key is ' + the_key);
      ...
      // if path_to_data is an array, get the specified data for each item in the array
        ...
        data_item.forEach(function(path_to_data) {
          ...
          var the_filename = path.basename(path_to_data, '.json');
