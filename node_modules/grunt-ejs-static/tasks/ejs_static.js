...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    function create_files(key, data_object) {
      ...
      // the page has no category and is therefore rendered at root level
        ...
        grunt.log.error('error in create_files() in ejs_static.js');
      }
    ...
    function path_to_layout_validator(key, data_object) {
      ...
      // if path_to_layout exists
        ...
        create_files(key, data_object);

      // if a layouts dir is specified
      } else if (options.path_to_layouts) {

        // search the layouts dir for a dir that matches key
        var search_pattern = options.path_to_layouts + '/**/' + key + '.ejs';
        grunt.log.debug('search pattern is ' + search_pattern);

        var fallback_layout_path = grunt.file.expand(search_pattern);
        grunt.log.debug('FALLBACK LAYOUT PATH IS ' + fallback_layout_path);
    ...
    function prepare_layout(key, data_object) {
      ...
      path_to_layout = options.dest + '/' + path_to_layout;
