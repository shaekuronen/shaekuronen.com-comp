  ...
  grunt.registerMultiTask('ejs_static', 'Render EJS templates as static HTML.', function() {
    ...
    if (options.path_to_data) {
      ...
      if (options.global_data) {
        var global_data = grunt.file.readJSON(options.path_to_global_data);
    ...
    function prepare_data(key, data_object, global_data, path_to_layout) {
      ...
      if (global_data) {
        grunt.log.debug('there is global_data');
        grunt.log.debug(global_data);

        var the_stringified_global_data = JSON.stringify(global_data, null, 2);
        grunt.log.debug('stringified global data is ' + the_stringified_global_data);

        the_data.global = global_data;

        grunt.log.debug('the concatenated data ' + JSON.stringify(the_data, null, 2));
      ...
      if (the_data.path_to_data) {
        ...
        grunt.log.debug('stringified OTHER data is ' + stringified_other_data);
        the_data[key] = other_data;

        grunt.log.debug('the concatenated data with the OTHER DATA is ' + JSON.stringify(the_data, null, 2));
      }
